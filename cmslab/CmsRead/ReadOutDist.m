function dist = ReadOutDist(outfile, core, str)
...
N=length(find(diff(iprista)>100))+1;
dist=cell(1,N);      % Number of state points
n=length(iprista)/N; % Number of printouts per state point
...
for i=1:N
    ...
    for j=1:n
        icoun=icoun+1;
        row=TEXT{iprista(i)+1};
        row(row=='*')=[];
        jcoor=sscanf(row,'%i');
        for i1=1:iafull,
            row=TEXT{iprista+1+i1};
            rowval=sscanf(row,'%g');
            icoor=rowval(1);rowval(1)=[];
            dist{i}(icoor,jcoor)=rowval;
        end
    end
end
%%         
