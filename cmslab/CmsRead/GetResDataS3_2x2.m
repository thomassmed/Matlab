function dists = GetResDataS3_2x2(resinfo,distlab,stptinp,asspos)
...
%                 dists.Xpo = resinfo.Xpo(stptinp);
        ...
        case 'LIBRARY'       
            ...
            % TODO: Fix so that all LIBRARY reads are made in one function
            ...
            Zzon=reshape(fueldata{2,2}',limzon+1,limfue);
            Nregin=reshape(fueldata{3,2}',limzon-1,limfue);
            [irg,jrg]=find(Nregin<0);
            for i=1:length(jrg),
                Nzon(:,jrg(i))=Nzon(:,-Nregin(irg(i),jrg(i)));
                Zzon(:,jrg(i))=Zzon(:,-Nregin(irg(i),jrg(i)));
            end
            ...
            nfta_list=[];
            Core_Seg=zeros(resinfo.core.kmax,length(nfta));
            Core_Seg2=Core_Seg;Core_Seg3=Core_Seg;Seg2_w=Core_Seg;Seg3_w=Core_Seg;
            for i=min(nfta):max(nfta);
                ii=find(nfta==i); %TODO fixa FUE.SUB
                if ~isempty(ii)
                    lim=Zzon(2:end,i);
                    ibort= lim==0;
                    lim(ibort)=[];
