function zi = xsinterp2(varargin)
...
% Check for out of range values of s and set to 1 Sme 2008-09-30
s_low=find(s<1);
shigh=find(s>ncols);
...
if ~isempty(s_low),
    ndx_s=ndx(s_low);t_s=t(s_low);
    dFds=arg3(ndx_s+nrows).*(1-t_s) + arg3(ndx_s+(nrows+1)).*t_s -...
          ( arg3(ndx_s).*(1-t_s) + arg3(ndx_s+1).*t_s );
    F(s_low)=F(s_low)+dFds.*dslow;
end

if ~isempty(shigh),
    ndx_s=ndx(shigh);t_s=t(shigh);
    dFds=arg3(ndx_s+nrows).*(1-t_s) + arg3(ndx_s+(nrows+1)).*t_s -...
          ( arg3(ndx_s).*(1-t_s) + arg3(ndx_s+1).*t_s );
...
if ~isempty(tlow),
    ndx_s=ndx(tlow);s_t=s(tlow);
    dFdt=-arg3(ndx_s+nrows).*s_t + arg3(ndx_s+(nrows+1)).*s_t -...
          arg3(ndx_s).*(1-s_t) + arg3(ndx_s+1).*(1-s_t) ;
    F(tlow)=F(tlow)+dFdt.*dtlow;
end

if ~isempty(thigh),
    ndx_s=ndx(thigh);s_t=s(thigh);
    dFdt=-arg3(ndx_s+nrows).*s_t + arg3(ndx_s+(nrows+1)).*s_t -...
          arg3(ndx_s).*(1-s_t) + arg3(ndx_s+1).*(1-s_t) ;
    F(thigh)=F(thigh)+dFdt.*dthigh;
end
