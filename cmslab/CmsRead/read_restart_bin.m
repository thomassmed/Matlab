function [fue_new,Oper,Title,Parameters,Dimensions,Derived_Terms,Fuel_Data,Grid,Segment,Core]=read_restart_bin(restart_file,XPO,nstop)
...
if nargin<3,  % Now we need to explore the whole file
    ...
    for i=1:length(itit)
        ...
        FORMAT{1,1}='*char';   FORMAT{1,2}='*char';   FORMAT{1,3}='float'; nr{1,3}=3;
                                                      FORMAT{2,3}='int=>int';
        next_record=get_next_record(fid,'TITLE   ',2000,FORMAT,nr);
...
Qnom=fue_new.hx*fue_new.hx*active_hcore*powden*kan;  % W
...
if strncmp(lwr,'BWR',3),
    ...
    fue_new.Wbyp_S3K=Wbyp_S3K;
    fue_new.modfld=modfld;
else % PWR
