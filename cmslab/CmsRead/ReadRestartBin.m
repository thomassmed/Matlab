function [fue_new,Oper,Title,Parameters,Dimensions,Derived_Terms,Fuel_Data,Grid,Segment,Core]=ReadRestartBin(restart_file,XPO,nstop)
% ReadRestartBin  Reads from binary restart file
%
...
%% testversion of a new algoritm to save time and perhapse more...
...
neof=ftell(fid);
fseek(fid,0,-1);
%% Get all positions of all labels
Labels = {'PARAMETERS'; 'TITLE'; 'DIMENSIONS'; 'DERIVED TERMS' ;'CORE                '; 'CUSTOM';'ITERATION';'DEPLETION';'SDCDATA';'FUEL              ';'GRID                ';'SEGMENT           ';'HYDRAULICS        ';'UNIFORM MDT       ';'AXIAL MDT       ';'BYPASS FLOW       ';'BYPASS VOID       ';'CORWIJ        ';'VOIDQUAL        ';'EPRIMOD       ';'UNITS       ';'HEAVY METAL    ';'FIXED MAPS        ';'CRD STP        ';'ROD                ';'DETECTOR  ';'VERSION 2.20    ';'VERSION 2.40    ';'VERSION 2.60    ';'ASSEMBLY DATA       ';'ASSEMBLY LABELS   ';'3D NODAL RPF    '};
pos_record = GetLabelPosFromBin(fid,Labels);

%% tar bort detta än så länge... 
% label='PARAMETERS    ';
% llab=length(label);
% read_block_size=1e7; % 10 MB should be a piece of cake for any semi-modern laptop
% if nargin==1, % If one argument is given, read only the first case
%     XPO=10000;
% end
% if nargin<3,  % Now we need to explore the whole file
%     if neof<read_block_size
%         blob=fread(fid,neof,'*char')';
%         blob(blob==255)=0; % Temporary work around for octave in linux
%         itit=strfind(blob,label);
%     else
%         offset=0;
%         itit=[];
%         for i=1:ceil(neof/(read_block_size-llab)),
%             blob=fread(fid,read_block_size,'uchar=>uchar')';
%             itit=[itit,strfind(blob,label)+offset];
%             offset=offset+read_block_size-llab;
%             fseek(fid,-llab,0); %just to ensure label get entirely inside a read blob
%         end
%         itit=unique(itit); % Should not be neccessary, but just as a safety measure
%     end
%     % Now find the right case
%     ifound=0;
%     for i=1:length(itit)
%         fseek(fid,itit(i),-1);
%         FORMAT=cell(2,3);nr=FORMAT; % Freemat compatible
%         FORMAT{1,1}='*char';   FORMAT{1,2}='*char';   FORMAT{1,3}='float'; nr{1,3}=3;
%                                                       FORMAT{2,3}='int=>int';
%         next_record=GetNextRecordtest(fid,'TITLE   ',2000,FORMAT,nr);
%         Title=next_record.data;
%         TITEL{i}=Title;
%         xpo(i)=Title{1,3}(1);
%         if abs(xpo(i)-XPO)<0.01, ifound=i;end
%     end
%     if XPO==10000, % TODO take care of information of exposure point
%         ifound=1;               % Assume S3 standard, first case
%     end
%     if XPO==20000, 
%         ifound=length(xpo);     % Assume S3 standard, last case
%     end
%     if ifound,
%         ioff=itit(1);
%         nstart=itit(ifound)-ioff;
%         if ifound<length(itit),
%             nstop=itit(ifound+1)-ioff;
%         else
%             nstop=neof;
%         end
%     else  % Deliberate exit, call was just enquiring
%         fue_new=[itit-itit(1),neof];
%         Oper=xpo;
%         Title=TITEL;
%         return
%     end
% elseif nargin==3, % Now we read between nstart and nstop with no further questions
%     nstart=XPO;
%     %nstop=nstop, as defined by input
% end
% fseek(fid,nstart,-1);
%% Parameters
clear FORMAT nr
...
next_record=GetNextRecord(fid,pos_record,1,FORMAT,nr);                                
% next_record=GetNextRecordtest(fid,'PARAMETERS',1000,FORMAT,nr,nstart,nstop);
...
next_record=GetNextRecord(fid,pos_record,2,FORMAT,nr);                                            
% next_record=GetNextRecordtest(fid,'TITLE',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
Title=next_record.data;
%% Dimensions
clear FORMAT nr
FORMAT=cell(2,4);nr=FORMAT;
FORMAT{1,1}='*char'; nr{1,1}=3;   FORMAT{1,2}='int'; nr{1,2}=12;   FORMAT{1,3}='*char';     FORMAT{1,4}='int=>int'; nr{1,4}=-1; 
FORMAT{2,1}='int'; nr{2,1}=4;                       
next_record=GetNextRecord(fid,pos_record,3,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'DIMENSIONS',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,4,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'DERIVED TERMS',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
search_str=['CORE                ',0];
next_record=GetNextRecord(fid,pos_record,5,FORMAT,nr);
% next_record=GetNextRecordtest(fid,search_str,10000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,6,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'CUSTOM',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,7,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'ITERATION',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,8,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'DEPLETION',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,9,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'SDCDATA',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,10,FORMAT,nr);
%  next_record=GetNextRecordtest(fid,'FUEL              ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,11,FORMAT,nr);
%     next_record=GetNextRecordtest(fid,'GRID                ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,12,FORMAT,nr);
%             next_record=GetNextRecordtest(fid,'SEGMENT           ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
% end
%                                      
% next_record=GetNextRecordtest(fid,'TABLES           ',1000,FORMAT,nr,nstart,nstop);
...
next_record=GetNextRecord(fid,pos_record,13,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'HYDRAULICS        ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,14,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'UNIFORM MDT       ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,15,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'AXIAL MDT       ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
%
%next_record=GetNextRecordtest(fid,'CORWIJ            ',1000,FORMAT,nr,nstart,nstop);
...
next_record=GetNextRecord(fid,pos_record,16,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'BYPASS FLOW       ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,17,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'BYPASS VOID       ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,18,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'CORWIJ        ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,19,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'VOIDQUAL        ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
FORMAT{1,4}='float'; nr{1,4}=-1;  FORMAT{1,5}='float'; nr{1,5}=-1;
next_record=GetNextRecord(fid,pos_record,20,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'EPRIMOD       ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
%%
clear FORMAT nr
FORMAT=cell(1,2);nr=FORMAT;
FORMAT{1,1}='int'; nr{1,1}=-1;   FORMAT{1,2}='int'; nr{1,2}=-1;   
next_record=GetNextRecord(fid,pos_record,21,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'UNITS       ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,22,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'HEAVY METAL    ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,23,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'FIXED MAPS        ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,24,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'CRD STP        ',1000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,25,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'ROD                ',100000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
FORMAT=cell(1,1);nr=FORMAT;
FORMAT{1,1}='int'; nr{1,1}=-1;  
next_record=GetNextRecord(fid,pos_record,26,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'DETECTOR  ',30000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,27,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'VERSION 2.20    ',30000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,28,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'VERSION 2.40    ',30000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,29,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'VERSION 2.60    ',30000,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
first_record=GetNextRecord(fid,pos_record,30,FORMAT,nr);
% first_record=GetNextRecordtest(fid,'ASSEMBLY DATA       ',rd_block_size,FORMAT,nr,nstart,nstop);
ico=ico+1;Lab{ico}=first_record.label;Adr(ico)=first_record.abs_pos;
...
fue_new.is=fue_new.iaf;
fue_new.js=fue_new.iaf;
...
label_record=GetNextRecord(fid,pos_record,31,FORMAT,nr);
% label_record=GetNextRecordtest(fid,'ASSEMBLY LABELS   ',rd_block_size,FORMAT,nr,nstart,nstop);
...
%     fseek(fid,ifind(i)+blk_size+(3+6)*i_int+ifloat,-1);
    fseek(fid,ifind(i)+blk_size+3*i_int,-1);
    fue_new.is(i)=fread(fid,1,'int32');
    fue_new.js(i)=fread(fid,1,'int32');
...
%     fseek(fid,ifind1(i)-10,-1);
    fseek(fid,ifind1(i),-1);
    next_record=GetNextRecord(fid,pos_record,31,FORMAT,nr,1);
%     next_record=GetNextRecordtest(fid,'ASSEMBLY LABELS',100,FORMAT,nr);
    ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
next_record=GetNextRecord(fid,pos_record,32,FORMAT,nr);
% next_record=GetNextRecordtest(fid,'3D NODAL RPF    ',1000,FORMAT,nr);
ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
...
% next_record=GetNextRecordtest(fid,'S3K BYPASS        ',1000,FORMAT,nr);
% ico=ico+1;Lab{ico}=next_record.label;Adr(ico)=next_record.abs_pos;
%%
if ~isempty(next_record.data),
