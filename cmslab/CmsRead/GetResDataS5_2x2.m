function dists = GetResDataS5_2x2(resinfo,distlab,stptinp,asspos)
...
if ischar(resinfo)
    ...
    elseif strcmpi(resinfo,'NONDISTS')
        nondists = {'OPER','ASMNAM','KEFF','NEW ASSEMBLY DATA', 'ASSEMBLY DATA','NUM MICRO DEP','SUBMESH DATA','ASSEMBLY ISOTOPICS','SUBMESH ISOTOPICS', 'ASSEMBLY LABELS','PINDATA','LIBRARY','THERMAL HYD','CONTROL ROD','NFTA DIST','NHYD DIST','KONROD','DIMS','FLK DATA'};
        dists = nondists;
        return
    end
end
...
if nargin == 4
    ...
    if ischar(asspos) 
        ...
        knumit=crnum2knum(knumit,resinfo.core.mminj2x2);
...
%             TODO: add for single assembly
            ...
            subnums = convert2x2('2vec',knum,mminj,sym);
            is = is(subnums);
            js = js(subnums);
            kbt = kbt(subnums);
            nft = nft(subnums);
        ...
        case {'XENON' ,'SAMARIUM','IODINE','PROMETHIUM','BURNUP','TFUHIST','CRDHIST','VHIST','DENHIST','BORONDENS','SURFEXPYM','SURFEXPXP','SURFEXPYP','SURFEXPXM','SPECHIST', 'SPECHISTYM','SPECHISTXP','SPECHISTYP','SPECHISTXM','EB1','BP1','BP10','BP2','BP20','EB2','XTF','FLN','FLNYM','FLNXP','FLNYP', 'FLNXM'}
            ...
            kan = GetSymKan(resinfo);
            kan = kan*4;
            coreind = convert2x2('2vec',knum,mminj,sym);
            if allass
                for i = 1:kan
                ...
                for i = 1:length(knumit)
                    [kpos ~] = find(resinfo.core.knum2x2 == knumit(i));
                    knums = coreind(kpos);
        ...
        case {'SUBNODE U235','SUBNODE U238','SUBNODE PU239','SUBNODE PU240','SUBNODE PU241','SUBMESH U235', 'SUBMESH U238', 'SUBMESH PU239', 'SUBMESH PU240', 'SUBMESH PU241'}
            ...
            if allass
                subdata = GetResDataS5_2x2(resinfo,'SUBMESH ISOTOPICS',stptinp);
            else
                subdata = GetResDataS5_2x2(resinfo,'SUBMESH ISOTOPICS',stptinp,asspos);
            end
        ...
        case 'PINDATA'
            ...
            if allass
                asslab = GetResDataS5_2x2(resinfo,'ASSEMBLY LABELS',stptinp);
                pinexp = GetResDataS5_2x2(resinfo,'PIN BURNUP',stptinp);
                geoms = GetResDataS5_2x2(resinfo,'SUBNODE GEOMETRY',stptinp);
                ...
                asslab = GetResDataS5_2x2(resinfo,'ASSEMBLY LABELS',stptinp,asspos);
                pinexp = GetResDataS5_2x2(resinfo,'PIN BURNUP',stptinp,asspos);
                geoms = GetResDataS5_2x2(resinfo,'SUBNODE GEOMETRY',stptinp,asspos);
                if iscell(geoms)
                    kdfuel = zeros(1,length(geoms));
                    for i = 1:length(geoms)
                        kdfuel(i) = length(geoms{i});
        ...
        case {'PIN BURNUP', 'PINEXP'}
            ...
            kan = kan*4;
            if allass
                cormap = convert2x2('2core',knum,mminj,sym);
                for i = 1:kan
                    fseek(fid,datapos(i),-1);
                    sizeread=fread(fid,2,'int');
                    fseek(fid,datapos(i)+sizeread(1)+12,-1);
        ...
        case 'EXP2D'
            ...
            if allass
                ...
                for i = 1:kan
                  ...
                  % TODO måste fixas med värden också!!  dists = convert2x2('2core',expv,resinfo.core.mminj);
                dists = expv;
        ...
        case 'ASSEMBLY LABELS'
            ...
            if allass
                for i = 1:kan
                    ...
                    dists.iaf = iaf;
                    dists.jaf = jaf;
                    dists.lab = nftid(coreind);
                    dists.ser = serial(coreind);
                    dists.nload = nload(coreind);
                    dists.nfra = nfra(coreind);                    
            else
                for i = 1:length(knumit)
                    [kpos pos(i)] = find(resinfo.core.knum2x2 == knumit(i));
                    knums = coreind(kpos);
        ...
        case {'IBAT' } 
            ...
            if allass
                ...
                for i = 1:length(knumit)
                    [kpos ~] = find(resinfo.core.knum2x2 == knumit(i));
                    knums = coreind(kpos);
                    fseek(fid,datapos(knums(i))+blk_size+3*i_int + 4*i_int+ifloat + 12,-1);
        ...
        case 'CONTROL ROD'
            ...
            Crdnam=Crdnam(~cellfun(@isempty,Crdnam));
            ...
            if all(crmminj==1)&&~all(resinfo.core.mminj==1)
                if resinfo.core.if2x2==2,
                    crmminj=resinfo.core.mminj;
                end
            end
        ...
        case {'KONROD','CRD.POS'}
            roddata = GetDist(resinfo,'ROD                 ',fid,offsetstpt);
            ctrdat = GetResDataS5_2x2(resinfo,'CONTROL ROD',stptinp);
        ...
        case 'POWER'
            ...
            beg = [1 2];
            kkan = 4*kkan;
            mminj = resinfo.core.mminj2x2;
            knum = resinfo.core.knum2x2;
        ...
        case 'OPER'
            ...
            Zzon=reshape(FuelData{2,2}',limzon+1,limfue);
            active_hcore=max(max(Zzon(:,nfta)));
            Qnom=dxassm*dxassm*active_hcore*powden*kan;
...
%             [~,sym]=ij2mminj(asslab.iaf,asslab.jaf,asslab.nload);
        ...
        case 'DIMS'
            ...
            nfta = GetResDataS5(resinfo,'NFTA',stptinp);
