function paint_btfp(~,~,hfig)

HotBirdProp=get(hfig,'userdata');
cn=HotBirdProp.cn;
axes(HotBirdProp.handles.patron);
reset(gca);
cmax = max(max(HotBirdProp.cs.btfp));
cmin = min(min(HotBirdProp.cs.btfp));
ncol = 9;
plmat=ncol*HotBirdProp.cs.btfp/(cmax-cmin)-ncol*cmin/(cmax-cmin)+1;
image(plmat,'ButtonDownFcn',{@mickey_patron,hfig});
set(gca,'XTick',[],'YTick',[]);

HotBirdProp.button=5;
...
for i=1:HotBirdProp.cs.npst
    for j=1:HotBirdProp.cs.npst
        if (HotBirdProp.cs.s(cn).enr(i,j) > 0 && HotBirdProp.cs.btfp(i,j) > 0)
            ce=sprintf('%4.2f',HotBirdProp.cs.btfp(i,j));
            text(j, i, ce,...
                'Color', 'black',...
                'FontSize', 10,...
                'FontWeight', 'bold',...
