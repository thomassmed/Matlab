...
%     cmax = max(max(HotBirdProp.cs.tmol));
%     cmin = min(min(HotBirdProp.cs.tmol));
%     plmat=ncol*HotBirdProp.cs.tmol/(cmax-cmin)-ncol*cmin/(cmax-cmin)+1;
% else
%     cmax = max(max(HotBirdProp.cs.s(cn).tmol));
%     cmin = min(min(HotBirdProp.cs.s(cn).tmol));
%     plmat=ncol*HotBirdProp.cs.s(cn).tmol/(cmax-cmin)-ncol*cmin/(cmax-cmin)+1;
% end
% 
...
%             if (HotBirdProp.cs.s(cn).enr(i,j) > 0 && HotBirdProp.cs.tmol(i,j) > 0)
%                 ce=sprintf('%4.2f',HotBirdProp.cs.tmol(i,j));
%                 text(j, i, ce,...
%                     'Color', 'black',...
%                     'FontSize', 10,...
%                     'FontWeight', 'bold',...
...
% else
    for i=1:HotBirdProp.cs.npst
        for j=1:HotBirdProp.cs.npst
            if (HotBirdProp.cs.s(cn).enr(i,j) > 0 && HotBirdProp.cs.s(cn).tmol(i,j) > 0)
                ce=sprintf('%4.2f',HotBirdProp.cs.s(cn).tmol(i,j));
                text(j, i, ce,...
                    'Color', 'black',...
                    'FontSize', 10,...
                    'FontWeight', 'bold',...
