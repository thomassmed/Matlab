function [conv,alfa,tl,Wg,Wl,tw]=syst_f4_new(fue_new,power,flowb,alfa,tl,Wg,Wl,tol)
%
%
% 
...
[ploss,y6]=eq_dP_core(alfa.*(alfa>0),tl,Wg,Wl,P,A,hz,flowb,vhk,Xcin,Dh,tlp); %  ncc
dPcore=mean(y6);
y6=y6-dPcore;
%}
%% dy1/dx
...
icount=icount+1;
%% eq_Wg_core
[a_c,tl_c,Wl_c]=fin_diff(@eq_Wg_core,alfa,tl,Wl,Wg,qprimw,p,A,hz,tlb,htc,pbm,phm,Dh);
iJ{icount}=[ibas+2   ibas+2    ibas+2   i_off_diag+2     ibas+2];
jJ{icount}=[ibas     ibas+1    ibas+2   i_off_diag+2-nt  ibas+3];
xJ{icount}=[a_c(:)'  tl_c(:)'    ett     -ett_o          Wl_c(:)'];
icount=icount+1;
%% Wl+Wg=chflow+flowb
iJP=zeros(1,ntot);
for i=1:ncc,
...
icount=icount+1;
%% Ploss
[a_c,tl_c,Wg_c,Wl_c]=fin_diff(@eq_dP_core,alfa,tl,Wg,Wl,flowb,P,A,hz,vhk,Xcin,Dh,tlp);
...
jJ=cat(2,jJ{:})';
xJ=real(cat(2,xJ{:})');
J=sparse(iJ,jJ,xJ,nt*kd+ncc+1,nt*kd+ncc+1);
[L,U]=lu(J);
%%
tot_flow=sum(chflow);
[ploss,y6]=eq_dP_core(alfa,tl,Wg,Wl,flowb,P,A,hz,vhk,Xcin,Dh,tlp);
dPcore=mean(real(y6));
%%
for i=1:10, 
    [ploss,y6]=eq_dP_core(alfa,tl,Wg,Wl,flowb,P,A,hz,vhk,Xcin,Dh,tlp);
