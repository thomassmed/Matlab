...
switch upper(NeuModel)
    case 'NEU1'
        [X1nm0,Y1nm0,X2nm0,Y2nm0]=eq_xy(neig,hx,hz,d1,d2,usig1,usig2,...
            siga1,sigr,siga2,a11,a21,a22,fa1,fa2,keff,cp);
        %-------------------------------------------------------
        % Calculate dqdfa1
        %-------------------------------------------------------
        %% Aqn
        ...
        [X1nm,Y1nm,X2nm,Y2nm]=eq_xy(neig,hx,hz,d1,d2,usig1,usig2,...
            siga1,sigr,siga2,a11,a21,a22,fa1,fa2,keff,cp,al,b,lam);
        A2nm=A_f12(X2nm,Y2nm,neig,siga2,sigr,regio);
        fa2c=A2nm*fa1;
        % Calculate X1nm,X2nm,Y1nm,Y2nm with fa2c rather than fa2
        [X1nm,Y1nm,X2nm,Y2nm]=eq_xy(neig,hx,hz,d1,d2,usig1,usig2,...
            siga1,sigr,siga2,a11,a21,a22,fa1,fa2c,keff,cp,al,b,lam);
        % Equation 36
...
%% NEU3     
    case 'NEU3'
        ...
        [Anm,r,raa,Sn,C1nm,C2nm,J1nm,J2nm,SC1,SC2]=eq_neu3(neig,hx,hz,d1,d2,usig1,usig2,siga1,...
           sigr,siga2,a11,a21,a22,psi,r0,keff,DF1,DF2,C1nm0,C2nm0);
       blvec=zeros(size(Xsec.d1));
       blvec(1104)=1;
       delta=0.0001;
        d1bl=Xsec.d1+delta*blvec.*Xsec.d1d.*drda;
        d2bl=Xsec.d2+delta*blvec.*Xsec.d2d.*drda;
        sigrbl=Xsec.sigr+delta*blvec.*Xsec.sigrd.*drda;
        siga1bl=Xsec.siga1+delta*blvec.*Xsec.siga1d.*drda;
        siga2bl=Xsec.siga2+delta*blvec.*Xsec.siga2d.*drda;
        usig1bl=Xsec.usig1+delta*blvec.*Xsec.usig1d.*drda;
        usig2bl=Xsec.usig2+delta*blvec.*Xsec.usig2d.*drda;
          [Anm12,r12,raa12,Sn12,C1nm12,C2nm12,J1nm12,J2nm12,SC112,SC212]=eq_neu3(neig,hx,hz,d1bl,d2bl,usig1bl,usig2bl,siga1bl,...
           sigrbl,siga2bl,a11,a21,a22,psi,r0,keff,DF1,DF2,C1nm0,C2nm0);
end
...
switch upper(NodalCode)
    ...
    case {'SIM3','POLCA7'}
        ...
        sigrbl=Xsec.sigr+delta*blvec.*Xsec.sigrd.*drda;
        siga1=Xsec.siga1+delta*blvec.*Xsec.siga1d.*drda;
        siga2bl=Xsec.siga2+delta*blvec.*Xsec.siga2d.*drda;
        usig1=Xsec.usig1+delta*blvec.*Xsec.usig1d.*drda;
        usig2=Xsec.usig2+delta*blvec.*Xsec.usig2d.*drda;
end
% First, the complex X1,Y1,X2,Y2 for use in the fa1-equation
[X1bl,Y1bl,X2bl,Y2bl]=eq_xy(neig,hx,hz,d1,d2,usig1,usig2,...
siga1,sigrbl,siga2bl,a11,a21,a22,fa1,fa2c,keff,cp,al,b,lam);
% Then, the real X1,Y1,X2,Y2 for use in the other equations
[X1bl0,Y1bl0,X2bl0,Y2bl0]=eq_xy(neig,hx,hz,d1,d2,usig1,usig2,...
siga1,sigrbl,siga2bl,a11,a21,a22,fa1,fa2,keff,cp);
...
switch upper(NodalCode)
    ...
    case {'SIM3','POLCA7'}
        ...
        siga1=Xsec.siga1+delta*whvec.*Xsec.siga1d.*drda;
        siga2wh=Xsec.siga2+delta*whvec.*Xsec.siga2d.*drda;
        usig1=Xsec.usig1+delta*whvec.*Xsec.usig1d.*drda;
        usig2=Xsec.usig2+delta*whvec.*Xsec.usig2d.*drda;
end
% First, the complex X1,Y1,X2,Y2 for use in the fa1-equation
[X1wh,Y1wh,X2wh,Y2wh]=eq_xy(neig,hx,hz,d1,d2,usig1,usig2,...
siga1,sigrwh,siga2wh,a11,a21,a22,fa1,fa2c,keff,cp,al,b,lam);
% Then, the real X1,Y1,X2,Y2 for use in the other equations
[X1wh0,Y1wh0,X2wh0,Y2wh0]=eq_xy(neig,hx,hz,d1,d2,usig1,usig2,...
siga1,sigrwh,siga2wh,a11,a21,a22,fa1,fa2,keff,cp);
