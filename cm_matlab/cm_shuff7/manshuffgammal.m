%@(#)   manshuffgammal.m 1.1	 05/07/13     10:29:34
%
%@(#)   manshuffgammal.m 1.1	 05/07/13     10:29:34
%
...
kedja;		%skriver ut kedja för att se
...
if isequal(mode,4),
 ...
 if goon>0,
   ...
   if pooluse,
     ...
     initpool(poolfil,'buidnt',hapool,hepool,htpool);
   end
   figure(curfig);
   setlabels;
   hand=get(gcf,'userdata');
...
% Skapar ett instruktionsfönster
   ...
   text(0.1,0.52,'Mittenknapp:   Angra senaste forflyttning');
   text(0.1,0.39,'Hoger utanfor hard: Kommentar');
    ...
    if knum>0,
      if fuel(knum)==0,
        ...
        if length(j)==0,
          ...
          ppos(nrop+1,:)='DMPOS';
          if pooluse,
	    clearpool(poolfil,buidboc(knum,:),hapool,hepool,htpool);
          end
	  lastoper=1;
        elseif length(j)==1,
      ...
      elseif fuel(knum)==1,
          ...
          lto(nrop+1)=0;
          if pooluse,
	    ppos(nrop+1,:)=getpoolpos(poolfil,buid(knum,:),hapool,hepool,htpool);
          else
	    ppos(nrop+1,:)='AB123';
	  end
	  lastoper=1;
    ...
    if knum>0,
      if nrop>0,
        ...
        if lto(nrop)~=0
          ...
          printsfg(0,lfrom(nrop),lto(nrop),lbuid(nrop,:),mminj,ppos(nrop,:),0,block);
          if pooluse,
	    clearpool(poolfil,lbuid(nrop,:),hapool,hepool,htpool);
          end
	end
        if lfrom(nrop)==0&pooluse,
	  clearpool(poolfil,lbuid(nrop,:),hapool,hepool,htpool,1);
	end
        figure(curfig);
        [gonu,from,to,ready,fuel,plmat,ssto]=updscreen(lfrom,lto,nrop,lbuid,...
        fram,newmove,skyffett,kedja,mode,mminj,ikan,to0,from0,ready0,gonu,from,to,ready,fuel,plmat);
...
if sparinfil==1,
  ...
  for i=1:nrop,
    if sstom(i,2)>0, 
      if lto(i)~=0
        printsfg(fid,lfrom(i),lto(i),'dummy',mminj,'',2,block);
      else
        printsfg(fid,lfrom(i),lto(i),'dummy',mminj,ppos(i,:),2,block);
      end
    end  
      if lto(i)~=0
    ...
    if sstom(i,1)>0, 
      if lto(i)~=0
        printsfg(fid,lfrom(i),lto(i),'dummy',mminj,'',1,block);
      else
        printsfg(fid,lfrom(i),lto(i),'dummy',mminj,ppos(i,:),1,block);
      end
    end  
    if ncomvec(i)==1,
      comcount=comcount+1;
      if lto(i)~=0
        printsfg(fid,0,0,'dummy',mminj,ppos(i,:),comment(comcount,:),block);
      end
    end
  end
  fclose(fid);
