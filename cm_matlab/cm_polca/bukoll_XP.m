function varargout = bukoll_XP(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%            bukoll_XP v1.0
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @bukoll_XP_OpeningFcn, ...
                   'gui_OutputFcn',  @bukoll_XP_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before bukoll_XP is made visible.
function bukoll_XP_OpeningFcn(hObject, eventdata, handles, varargin)
...
% UIWAIT makes bukoll_XP wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = bukoll_XP_OutputFcn(hObject, eventdata, handles)
...
[STARTfil,STARTvag]=uigetfile({'*.dat'},'OBS! Härdinventariefil från LADDA');
...
elseif strcmp(neumodel,'POLCA4'),
  ...
  [crid]=readdist(DISTRIBUTIONSfilen,'cridnt');
    ...
    fprintf(resultat,'\n\n HÄRDINVENTARIE KONTROLL utförd med bukoll_XP ');
    fprintf(resultat,'\n Utförd: %s ',date);
    ...
    fel_med={' ','bukoll_XP stötte på ett problem',' ','Troliga fel kan vara:',' ','Filerna är inte av rätt typ POLCA4/POLCA7.','Du har försökt att köra programmet i fel miljö (Unix/Linux).'};
    errordlg(fel_med,'Fel meddelande bukoll_XP','bukoll_XP problem...');
    disp('------------------------------------------')
...
Instr_text={' ','ALLMÄNT:','bukoll_XP kontrollerar om den angivna härdinventariefilen från LADDA stämmer överens med de patronidentiteter som POLCA anser ska finnas i härden.',' ','ANVÄNDNING:','En härdinventariefil från LADDA väljs som första fil och sedan väljs den aktuella BOC-filen man vill jämföra med. Om filerna skiljer sker en utskrift på skärmen som visar vilka positioner som skiljer.',' ','Utskrift:','Om man så önskar kan man välja att skriva ut resultatet antingen genom att skriva ut filen direkt från nedit eller via normal utskrift från prompt.',' ','ÖVRIGT:','Om du har frågor om programmet kontakta då i första hand:',' ',' Jan Karjalainen, FTB'};
helpdlg(Instr_text,'bukoll_XP instruktion');
% --------------------------------------------------------------------
function om_bukoll_Callback(hObject, eventdata, handles)
Om_text={' ','                                            bukoll_XP',' ','Bukoll_XP kontrollerar om LADDA:s och POLCA:s härdpositioner överenstämmer.',' ','Ingen kopiering eller vidarespridning av bukoll_XP v1.0 får ske utan','Forsmarks Kraftgrupp AB:s skriftliga godkännande.', ' ','         © Forsmarks Kraftgrupp AB, 2005, All rights reserved'};
helpdlg(Om_text,'Om bukoll_XP');
%***********************************************************
