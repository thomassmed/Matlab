%@(#)   orecomp.m 1.1	 09/11/13     15:30:34
%
...
for row=rowa:rowb
   ...
   if (strcmp(upper(dm),'DM') & strcmp(ch(ll-1:ll),'DM')) | ~strcmp(upper(dm),'DM')
      [cnums,patts]=orepatts(supfil,row,refdistfile,cmanfil,sym);
      crnums=[crnums;cnums];
      ...
      for ipatt=1:size(patts,1)
         ...
         if ind==1,i=bucatch('CONROD',tx(:,1:6))+1;
            ...
            tx(i(1)-5,1:48)=sprintf('%s%5.0f','TITLE  Kontroll av fallande stav i monster ',pattvec(row));
