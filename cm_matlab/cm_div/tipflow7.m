%@(#)   tipflow.m 1.10	 94/05/31     10:15:00
%
%function [normchdiff,normchdiffc,date,tipfiles]=tipflow7(chmeas,chflow,efph,plotflokan,tipfiler)
%compares measured channel flow with computed
%Input:
%      chmeas     - measured chflow (output from evalpolca7)
%      chflow     - computed chflow (output from evalpolca7)
%      efph       - efph at TIP (output from evalpolca7)
...
%
% example:
%              tipflow7(chmeas,chflow,efph) - presents no plot, only printout
%              tipflow7(chmeas,chflow,efph,3) - plots the third (in alphabetical order) "K-punkt"
%              tipflow7(chmeas,chflow,efph,(1:8)) - plots all "K-punkter"
%              tipflow7  -  First evalpolca is executed, then tipflow is executed
%
function [normchdiff,normchdiffc,date,tipfiles]=tipflow7(chmeas,chflow,efph,plotflokan,tipfiler)
reakdir=findreakdir;
...
for i=1:length(plotflokan)
  ...
  title(['POLCA7  Flödesavvikelse for ',knam(plotflokan(i),:)]);
end  
