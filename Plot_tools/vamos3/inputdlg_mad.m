...
for lp=1:NumQuest,
    ...
    if size(NumLines,2) == 2,
        EditWidth = setcolumnwidth(EditHandle(lp), NumLines(lp,2));
        MinWidth = max(MinWidth, EditWidth);
    end
    FigWidth=max(FigWidth, MinWidth+DefOffset);
...
set(InputFig,'Position',getnicedialoglocation_mad(FigPos,get(InputFig,'Units')));
...
set(InputFig,'ResizeFcn', {@doResize, size(NumLines,2) == 2});
...
function EditWidth = setcolumnwidth(object,width)
...
set(object, 'String', char(ones(1,width)*'x'));

new_extent = get(object,'Extent');
new_position = old_position;
new_position(3) = new_extent(3);
